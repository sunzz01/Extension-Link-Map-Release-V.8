...
export const TabNodeOperations = {
    ...
    updatePartial(toUpdateNode: FancytreeNode, updateProps: Partial<TabData>) {
        ...
        const prevAudible = toUpdateNode.data.audible;
        const prevMuted = toUpdateNode.data.mutedInfo?.muted;
        ...
        // Check for audio state changes to trigger re-render
        const newAudible = toUpdateNode.data.audible;
        const newMuted = toUpdateNode.data.mutedInfo?.muted;
        
        if (newAudible !== undefined && (newAudible !== prevAudible || newMuted !== prevMuted)) {
            toUpdateNode.renderTitle();
        }
    },
