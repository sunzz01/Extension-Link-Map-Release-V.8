...
const { TYPE_ATTR, NODE_CLOSE, NODE_REMOVE, NODE_EDIT, NODE_MUTE_TOGGLE } = TPL_CONSTANTS;
...
export class FancyTabMasterTree {
    ...
    constructor(
        ...
        $container.fancytree({
            ...
            enhanceTitle: (_eventData: JQueryEventObject, data: Fancytree.EventData) => {
                const html = renderTitle(_eventData, data, config.enableEdit, this.settings?.showUrl);
    ...
    public async initTree(source?: TreeNode<TreeData>[]) {
        ...
        if (this.enablePersist) {
            ...
            // Real-time audio status sync - poll every 500ms
            setInterval(async () => {
                try {
                    const allTabs = await browser.tabs.query({});
                    allTabs.forEach((tab) => {
                        if (tab.id === undefined) return;
                        const node = this.tree.getNodeByKey(`${tab.id}`);
                        if (!node) return;
                        
                        const prevAudible = node.data.audible;
                        const prevMuted = node.data.mutedInfo?.muted;
                        const newAudible = tab.audible;
                        const newMuted = tab.mutedInfo?.muted;
                        
                        // Only update if audio state changed
                        if (prevAudible !== newAudible || prevMuted !== newMuted) {
                            node.data.audible = newAudible;
                            node.data.mutedInfo = tab.mutedInfo;
                            node.renderTitle();
                        }
                    });
                } catch (e) {
    ...
    public async activeTab(windowId: number, tabId: number): Promise<void> {
        // devtools的windowId为-1，不做处理
        const tabNode = this.tree.getNodeByKey(`${tabId}`);
        if (windowId < 0 || !tabNode) return;

        try {
            // Fetch latest tab info to sync audio status
    ...
    private async syncActiveTab(windowId: number): Promise<boolean> {
        ...
        // Update with full details from the query
        const tabNode = this.tree.getNodeByKey(`${activeTab.id}`);
        if (tabNode) {
            TabNodeOperations.updatePartial(tabNode, {
                active: true,
...
function renderTitle(
    ...
    const treeNode = new TreeNodeTpl(data.node, enableBtnGroup, showUrl);
...
FancyTabMasterTree.onClick = (event: JQueryEventObject, data: Fancytree.EventData): boolean => {
    ...
    switch (target.attr(TYPE_ATTR)) {
        ...
        case NODE_EDIT:
            ...
            data.node.editStart();
            break;
        case NODE_MUTE_TOGGLE: {
            log.debug('[tree]: mute toggle clicked');
            const { id, mutedInfo } = data.node.data;
            const currentMuted = mutedInfo?.muted ?? false;
            browser.tabs.update(id, { muted: !currentMuted });
            event.preventDefault();
            return false;
        }
    }
