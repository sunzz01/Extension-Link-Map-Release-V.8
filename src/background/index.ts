...
try {
    ...
    async function initializeSidePanel() {
        try {
            // Type assertion for chrome.sidePanel API
            const chromeWithSidePanel = chrome as typeof chrome & {
                sidePanel?: {
            ...
            if (chromeWithSidePanel.sidePanel?.setPanelBehavior) {
                // Configure side panel to open when action button is clicked
                await chromeWithSidePanel.sidePanel.setPanelBehavior({ 
                    openPanelOnActionClick: true 
                });
                
                // Set panel options
                if (chromeWithSidePanel.sidePanel.setOptions) {
                    await chromeWithSidePanel.sidePanel.setOptions({
                        path: 'tree.html',
                        enabled: true
                    });
                }
                
                log.debug('Side panel initialized successfully');
            }
        } catch (error) {
            log.warn('Side panel initialization failed, will use popup fallback:', error);
        }
    }
    ...
    // Fallback: If side panel doesn't work, use popup window
    // This listener will only fire if side panel is not configured
    browser.action.onClicked.addListener(async (tab) => {
        try {
            ...
            // If side panel API is not available, use popup window fallback
            if (!chromeWithSidePanel.sidePanel) {
                setPrevFocusWindowId(tab.windowId!);
                await focusOrCreateExtWindow();
            }
        } catch (error) {
            log.error('Error in action click handler:', error);
            setPrevFocusWindowId(tab.windowId!);
            await focusOrCreateExtWindow();
        }
    });
